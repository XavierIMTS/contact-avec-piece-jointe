...
// clé secrete google recaptcha https://www.google.com/recaptcha/admin/site/
// $secret="6LeGjCojAAAAAKu449zg0fOsbjsnbZ5UTdZjBK1b"; clef pour backhub.fr
// vous devez créer vos clef pour votre domain

// on récupère la clef secrète dans le champ caché du formulaire
if(isset($_POST['secret']) ){
	$secret=$_POST['secret'];
	// on charge la librairie recapcha fourni par google
	require_once '../vendor/recaptcha/autoload.php';
	//on crée une instance de la class ReCaptcha
	$recaptcha = new \ReCaptcha\ReCaptcha($secret);
}

	
// on vérifie si le recaptcha rempli ou non
if(isset($_POST['g-recaptcha-response']) ){
	
	$resp = $recaptcha->setExpectedHostname($_POST['websitename']) // ici le sous domaine doit être renseigner ex: 2022.tests.backhub.fr
					  ->verify($_POST['g-recaptcha-response']);
	if ($resp->isSuccess()) {
		echo "<div class='recaptcha_verified' >Recaptcha verifié pour ".$_POST['websitename']."</div>";// Verified!
	} else {
		$errors = $resp->getErrorCodes();
		echo "<div class='recaptcha_not_verified' >Recaptcha non validé : ".$_POST['websitename']."<br>".printf($errors[0])."</div>";// Verified!

		return; // on execute pas le code suivant de traitement du formulaire si la condition recaptcha n'est pas remplie.
	}
}
...
if(isset( $_POST['email_sender'])) {
	$MAIL['envoyer_a']= $_POST['email_sender'];//(destinataire) mettez votre adresse mail ou l'adresse mail d'un autre destinataire (pour envoyer à plusieurs dstinataires, séparez par une virgules les adresses)
}

if(isset($_POST['email_subject']) && isset($_POST['email_sender'])){
	$MAIL['sujet']= $_POST['email_subject']." de ".$MAIL['envoyer_a'];
}
...
if(isset($_POST['envoyer'])){// si le bouton "Envoyer" est appuyé
	...
	} elseif(empty($_POST['nom'])){//on vérifie que le champ nom n'est pas vide
	...
	function errormail(){
		echo "erreur d'envoie de email".$this->$MAIL['prepare_mail']->error;
